...
int main(void)
{
    ...
    /*Initialize smif from secure domain*/ 
    ...
    /*Initialize MemNum for non secure access*/
    result = Cy_SMIF_MemNumInit(SMIF0_CORE, &smif0BlockConfig, &mem_context);
    if (CY_RSLT_SUCCESS != result)
    {
        handle_app_error();
    }

    /*Apply peripheral security configuration*/
    Cy_PPC0_Init();
    Cy_PPC1_Init();
